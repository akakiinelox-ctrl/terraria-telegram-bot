...
async def calc_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "🧮 **ИНЖЕНЕРНЫЙ ЦЕХ v2.0**\n\nВыбери инструмент для точного расчета ресурсов:",
...
async def gob_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "💰 **Введите цену перековки (в золоте):**\n_Пример: 15.5_",
        reply_markup=builder.as_markup(),
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def gob_res(message: types.Message, state: FSMContext):
    try:
        ...
        res = (
            f"💰 **Результаты перековки:**\n"
            f"━━━━━━━━━━━━━━\n"
            f"😐 **База:** {p}г\n"
            f"😊 **С Механиком (17%):** `{round(p*0.83, 2)}`г\n"
            f"❤️ **Макс. счастье (33%):** `{round(p*0.67, 2)}`г\n\n"
            f"💡 _Совет: Чтобы получить 33%, Гоблин должен жить в Пещерах с Механиком и Красильщиком._"
        )
        builder = InlineKeyboardBuilder()
...
async def ore_input(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"🔢 **Сколько слитков ({name}) тебе нужно?**",
        reply_markup=builder.as_markup(),
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def ore_res_calc(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            f"✅ Для **{bars}** слитков тебе понадобится **{total}** ед. руды _{data['ore_name']}_.",
            reply_markup=builder.as_markup(),
            parse_mode="Markdown"
        )
        await state.clear()
...
async def armor_res_final(callback: types.CallbackQuery):
    ...
    text = (f"🛡️ **Комплект: {name}**\n"
            f"━━━━━━━━━━━━━━\n"
            f"📦 Требуется: **{count}** ед. материала\n\n"
            f"🧩 _Обычно это:\n— Шлем: ~12-15\n— Нагрудник: ~20-24\n— Поножи: ~15-18_")
    builder = InlineKeyboardBuilder()
