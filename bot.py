...
from groq import Groq  # Подключаем Groq для ИИ-Гида
...
# НОВОЕ: Состояние для чата с Гидом
...
async def call_groq_guide(message_to_edit: types.Message, query: str, detail_level: str):
    try:
        system_instruction = (
            "Ты — Гид (Guide) из игры Terraria. Ты добрый и мудрый наставник. "
            "Ты обращаешься к игроку на 'ты', называешь его 'Террариец'. "
            "Твоя цель — помочь выжить. Если спрашивают о крафте — распиши его. "
            "Отвечай на ЛЮБЫЕ вопросы игрока, но всегда оставайся в роли Гида. "
        )
        
        if detail_level == "high":
            system_instruction += " Сейчас ответь МАКСИМАЛЬНО подробно, со всеми нюансами и секретами."
        ...
        await message_to_edit.edit_text("🛰️ **Гид:** Кажется, я потерял мысль... Попробуй ещё раз.")
...
async def chat_with_guide_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("👋 **Гид:** Привет! О чём ты хочешь поговорить? Спрашивай о чём угодно!")
...
async def guide_answer(message: types.Message, state: FSMContext):
    ...
    sent_msg = await message.answer("🤔 *Гид думает...*")
...
async def guide_more_info(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not last_query:
        await callback.answer("Я забыл, о чём мы говорили...", show_alert=True)
        return
    await callback.answer("Углубляюсь в детали...")
    await call_groq_guide(callback.message, f"Расскажи максимально подробно про: {last_query}", detail_level="high")
